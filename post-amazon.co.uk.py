...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/amazon.co.uk-backlinks_pages.xlsx')
    ...
    for i in range(60, 300):
        ...
        if '/dp/' in __url:
            if 'edit-video' in __url:
                ...
                __title = __title.replace("Amazon.co.uk", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-amazon_uk.txt', str(__url))
...
def browser():
    ...
    recaptcha(driver, '//input[@id="sp-cc-accept"]')
    time.sleep(4)
    for item in list_indeed:
        ...
        if 'Appstore for Android' in getinnertextXpath(driver, '//title'):
            pass
        else:    
            myDict["TITRE"] = getinnertextXpath(driver, '//span[@id="productTitle"]') + ' By ' + getinnertextXpath(driver, '//a[@id="bylineInfo"]')
            myDict["url"] = item["__url"]
            myDict["DESCRIPTION"] = getinnertextXpath(driver, '//div[@class="a-expander-partial-collapse-container"]')[0]
            myDict["CATEGORY"] = getinnertextXpath(driver, '//*[@id="nav-subnav"]/a[1]/span')
            if myDict["CATEGORY"] == ' ':
                myDict["CATEGORY"] = getinnertextXpath(driver, '//select[@aria-describedby="searchDropdownDescription"]//option[@selected="selected"]')
            myDict["YEAR"] = '2022'
            try:
                img_src = driver.find_element(By.XPATH, '(//div[@id="imgTagWrapperId"]//img)')
                img_src = img_src.get_attribute('src')
            except NoSuchElementException:
            ...
            if '" "' in myDict["IMAGE1"]:
                ...
                try:
                    img_src = driver.find_element(By.XPATH, '(//div[@id="imgTagWrapper"]//img)')[0]
                    img_src = img_src.get_attribute('src')
                except NoSuchElementException:
                    img_src = item["__title"]
                    pass
                ...
                append_new_line(r'result-amazon_uk.txt', str(myDict))
                final_result.append(myDict)


    with open("res-amazon_uk.json", "wb") as writeJSON:
