...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/reviewsbird.com-organic.Positions-us-20230210-2023-02-12T00 51 18Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(3, 200):
        __url = str(df.at[i, 'URL'])
        if '-reviews' in __url and int(df.at[i, 'Keyword Difficulty']) < 47: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("reviewsbird", " " )
                ...
                print(list_search)
                append_new_line(r'link-reviewsbird.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://reviewsbird.com/")
    time.sleep(8)
    ...
    for item in list_indeed:
        ...
        time.sleep(5)
        recaptcha(driverinstance, '//button[@class="swal2-close"]')
        myDict = {}
        myDict["TITRE"] = remove_tags(getinnertextXpath(driverinstance, '//h1'))
        if myDict["TITRE"] == ' ':
            myDict["TITRE"] = item["__title"]
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//h2[3]/following-sibling::p/following-sibling::p'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = item["__description"]
        myDict["CATEGORY"] =  find_between(item["__url"], 'https://reviewsbird.com/', '/')
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = 'StartUp'
        myDict["CATEGORY"] += ' , Compagnie,  Company, Entreprises'
        myDict["YEAR"] = '2022'
        tempp= item["__url"] + '/out/company-logo'
        myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//a[contains(@href, '"+tempp+"')]//img", 'src')), item["__img"])
        ...
        append_new_line(r'result-reviewsbird.txt', str(myDict))
        final_result.append(myDict)


    with open("res-reviewsbird.json", "wb") as writeJSON:
