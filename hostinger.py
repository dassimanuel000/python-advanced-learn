...
def post_commet(driverinstance, link, url):
    ...
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@name, "email")]') == 0:
        try:
            tryAndRetryFillByXpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@name, "email")]', str('contact@life-community.fr'))
        except NoSuchElementException:
            pass
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@name, "author")]') == 0:
        try:
            tryAndRetryFillByXpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@name, "author")]', str('French Life Community'))
        except NoSuchElementException:
            pass
    ...
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@name, "securitycode")]') == 0:
        try:
            driverinstance.find_element(By.XPATH, '//form[contains(@id, "comment")]//input[contains(@name, "securitycode")]').send_keys(str('French Life Community'))
        except NoSuchElementException:
            pass
...
def publish():
    ...
    dernier_etape_processus = "site-link-step.txt"
    try:
    ...
    for item in range(int(etape_processus), 8000):
        ...
        #WORDS = "zeru ca .pdf"
        title = WORDS
        linkkk = linecache.getline(r"./link-site.txt", item)
