...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/trustpilot.com-backlinks_pages (1).xlsx')
    # Entrer les lignes à scrap
    for i in range(330, 710):
        ...
        if '/review/' in __url:
            if 'edit-video' in __url:
                ...
                __title = __title.replace("| Read Customer Service Reviews of", " " )
                __title = __title.replace(" | Lisez les avis clients de ", " " )
                __title = __title.replace(" Les kundenes anmeldelser av ", " " )
                __title = __title.replace(" | Læs kundernes ", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-trustpilot.txt', str(__url))
...
def browser():
    ...
    for item in list_indeed:
        ...
        myDict = {}
        try:
            myDict["TITRE"] = getinnertextXpath(driver, '//h1')
        except NoSuchElementException:
            myDict["TITRE"] = getinnertextXpath(driver, '//span[contains(@class, "styles_websiteUrl")]')
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "customer-generated-content")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[@id="business-unit-title"]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//li[1]//a[contains(@class, "breadcrumb_breadcrumbLink")]'))#//nav[aria-label="Breadcrumb"]
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//a[contains(@href, "/categories/")]'))
        myDict["YEAR"] = '2022'
        try:
            wait = WebDriverWait(driver, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//img[contains(@class, "business-profile-image_image")]')))
                img_src = img_src.get_attribute('src')
            except NoSuchElementException:
                pass
        except:
        ...
        myDict["VIDEO"] = ''
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driver, "//ul[contains(@class, 'styles_contactInfoAddressList')]"))
        if myDict["LOCATION"] == ' ':
            myDict["LOCATION"] = remove_tags(getinnertextXpath(driver, '//div[@data-automation="breadcrumbs"]//div[1]//a//span'))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driver, "//time[contains(@data-service-review-date-time-ago, 'true')]"))
        try:
            time.sleep(4)
            tryAndRetryClickXpath(driver, '//input[@id="star-filter-page-filter-one"]')
            driver.find_element(By.ID, '').click()
            time.sleep(10)
            num1 = random.randint(1, 6)
            num1 = str(num1)
            try:
                myDict["author"] =  getinnertextXpath(driver, '//span[@data-consumer-name-typography="true"]['+num1+']')
            except NoSuchElementException:  #spelling error making this code not work as expected
                pass
            
            try:
                myDict["title"]= getinnertextXpath(driver, '//h2[@data-service-review-title-typography="true"]['+num1+']')
            except NoSuchElementException:
                pass

            try:
                myDict["comment"] =  getinnertextXpath(driver, '//p[@data-service-review-text-typography="true"]['+num1+']')
            except NoSuchElementException: 
                pass
        except NoSuchElementException:
            pass
        append_new_line(r'result-trustpilot.txt', str(myDict))
        final_result.append(myDict)


    with open("res-trustpilot.json", "wb") as writeJSON:
