...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/avis-verifies.com-organic.Positions-fr-20230103-2023-01-04T11 25 47Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(22, 2600):
        __url = str(df.at[i, 'URL'])
        if '/avis-clients/' in __url and int(df.at[i, 'Keyword Difficulty']) < 57 and int(df.at[i, 'Position']) < 40: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("avis-verifies", " " )
                __title = __title.replace(" | Reviews, Pricing, Features", " " )
                __title = __title.replace(" | avis-verifies", " " )
                __img = ""
                __description = __title + ", "+ __title +" : définition     Les caractéristiques des "+ __title +"     Le business plan de "+ __title +"     Le financement de "+ __title +"     La forme juridique de "+ __title +""
                list_search.append({'__url': __url, '__title': __title, '__img': __img, '__description': __description})
                print(list_search)
                append_new_line(r'link-avis-verifies.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://avis-verifies.com/")
    time.sleep(8)
    ...
    for item in list_indeed:
        ...
        myDict = {}
        if '/e/' in item["__url"]:
            myDict["TITRE"] = remove_tags(getinnertextXpath(driverinstance, '//span[contains(@itemprop, "name")][1]'))
            if myDict["TITRE"] == ' ':
                myDict["TITRE"] = item["__title"]
            myDict["url"] = item["__url"]
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//p[contains(@class, "text-justify")][1]'))
            if myDict["DESCRIPTION"] == ' ':
                myDict["DESCRIPTION"] = item["__description"]
                
            myDict["CATEGORY"] = ' , Compagnie,  Company, Entreprises'
            myDict["YEAR"] = '2022'
            
            myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//a[contains(@class, 'logos')]//img", 'src')), item["__img"])
                
            myDict["VIDEO"] = remove_tags(getinnertextXpath(driverinstance, '//iframe[contains(@src, "youtube")]'))
            
            myDict["WEBSITE"] = valueifnull(remove_tags(findATTR(driverinstance, "//a[contains(@class, 'logos')]", 'href')), " ")
            myDict["EMAIL"] = remove_tags(getinnertextXpath(driverinstance, '//span[contains(@style, "font-size:12px;word-wrap: break-word;")]'))
            myDict["PHONE"] =  remove_tags(getinnertextXpath(driverinstance, '//span[contains(@itemprop, "telephone")][1]'))
            myDict["FACEBOOK"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "facebook.com")][1]'))
            myDict["TWITTER"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "instagram.com")][1]'))
            myDict["INSTAGRAM"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "twitter.com")][1]'))
            myDict["LINKEDIN"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "linkedin.com")][1]'))
            myDict["LOCATION"] =  remove_tags(getinnertextXpath(driverinstance, '//span[contains(@itemprop, "streetAddress")][1]'))+ remove_tags(getinnertextXpath(driverinstance, '//span[contains(@itemprop, "postalCode")][1]'))+ remove_tags(getinnertextXpath(driverinstance, '//span[contains(@itemprop, "addressLocality")][1]'))
            ...
            if myDict["TITRE"] == ' ':
                myDict["TITRE"] = item["__title"]
            myDict["url"] = item["__url"]
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "attestation__content-wrapper")][2]//ul//li[2]//p[2]'))
            if myDict["DESCRIPTION"] == ' ':
                myDict["DESCRIPTION"] = item["__description"]
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//p[contains(@id, "information-category")]//a'))
            myDict["CATEGORY"] += ' , Compagnie,  Company, Entreprises'
            myDict["YEAR"] = '2022'
            
            myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//div[contains(@class, 'rating__logo')]//img", 'src')), item["__img"])
                
            myDict["VIDEO"] = remove_tags(getinnertextXpath(driverinstance, '//iframe[contains(@src, "youtube")]'))
            
            myDict["WEBSITE"] = valueifnull(remove_tags(findATTR(driverinstance, "//p[contains(@id, 'information-website')]//a", 'href')), " ")
            myDict["EMAIL"] = ""
            myDict["PHONE"] =  remove_tags(getinnertextXpath(driverinstance, '//span[contains(@itemprop, "telephone")][1]'))
            myDict["FACEBOOK"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "facebook.com")][1]'))
            myDict["TWITTER"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "instagram.com")][1]'))
        ...
        append_new_line(r'result-avis-verifies.txt', str(myDict))
        final_result.append(myDict)


    with open("res-avis-verifies.json", "wb") as writeJSON:
