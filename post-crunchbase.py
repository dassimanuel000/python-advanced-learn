...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/immodvisor.com-organic.Positions-fr-20230210-2023-02-12T00 01 08Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(1, 685):
        __url = str(df.at[i, 'URL'])
        if '/organization/' in __url: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("immodvisor", " " )
                ...
                print(list_search)
                append_new_line(r'link-immodvisor.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://www.immodvisor.com")
    time.sleep(8)
    ...
    for item in list_indeed:
        ...
        myDict["CATEGORY"] += ', Agence Immobiliere, Real Estate, Apartments, Houses,'
        myDict["YEAR"] = '2022'
        ...
        append_new_line(r'result-immodvisor.txt', str(myDict))
        final_result.append(myDict)


    with open("res-immodvisor.json", "wb") as writeJSON:
