...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/b-reputation.com-organic.Positions-fr-20230103-2023-01-04T11 24 54Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(10, 150):
        __url = str(df.at[i, 'URL'])
        if '/public/' in __url and int(df.at[i, 'Traffic']) > 20:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("b-reputation", " " )
                __title = __title.replace(" | Reviews, Pricing, Features", " " )
                __title = __title.replace(" | b-reputation", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-b-reputation.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    driverinstance.get("https://www.b-reputation.com")
    recaptcha(driverinstance, '//button[@class="cc-nb-okagree"]')
    time.sleep(4)
    input('/////////////////////////')
    for item in list_indeed:
        ...
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = getinnertextXpath(driverinstance, '//h1//span//span[contains(@class, "font-22")]')
        if myDict["TITRE"] == "www.b-reputation.fr":
            myDict["TITRE"] = item["__title"].split(',')[0]
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "company-description")]//div'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "company-custom-description")]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//span[contains(@class, "bg-expertise")][1]//a')) + ',' +  remove_tags(getinnertextXpath(driverinstance, '//span[contains(@class, "bg-expertise")][2]//a'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "/fr/top/")]'))
        ...
        try:
            ...
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//div[contains(@class, "company-logo")]//img[contains(@class, "p-3")]')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        myDict["INSTAGRAM"] = ''
        myDict["LINKEDIN"] = ''
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "company-info")]//div[contains(@class, "m-0")]//p'))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driverinstance, '//li[contains(@href, "creationDate")]//span[2]'))
        
        append_new_line(r'result-b-reputation.txt', str(myDict))
        final_result.append(myDict)


    with open("res-b-reputation.json", "wb") as writeJSON:
