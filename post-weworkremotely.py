...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/weworkremotely.com-backlinks_pages.xlsx')
    ...
    for i in range(10, 100):
        ...
        if '/reviews' in __url or '/news' in __url:
            ...
            __title = __title.replace(" – weworkremotely? News", "" )
            __title = __title.replace(" - weworkremotely.", " " )
            list_search.append({'__url': __url, '__title': __title})
            print(list_search)
            append_new_line(r'link-weworkremotely.txt', str(__url))
...
def browser():
    ...
    driver.get("https://www.weworkremotely.com/")
    recaptcha(driver, '//div[@id="adroll_allow_all"]')
    for item in list_indeed:
        ...
        myDict["LOCATION"] = ''
        myDict["CREATED_AT"] = ''
        append_new_line(r'result-weworkremotely.txt', str(myDict))
        final_result.append(myDict)


    with open("res-weworkremotely.json", "wb") as writeJSON:
