...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/W/medoucine.com-organic.Positions-fr-20230408-2023-04-09T18 25 23Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(1, 5150):
        __url = str(df.at[i, 'URL'])
        if '/consultation/' in __url and int(df.at[i, 'Keyword Difficulty']) < 32 and int(df.at[i, 'Search Volume']) > 460 and int(df.at[i, 'Position']) < 50:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("medoucine", " " )
                ...
                print(list_search)
                append_new_line(r'link-medoucine.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://www.medoucine.com")
    time.sleep(8)
    ...
    for item in list_indeed[28:]:
        ...
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@id, "session-benef-text")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = item["__description"]
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "tag--lg")][1]'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "tag--lg")][2]'))
        myDict["CATEGORY"] = ' Therapeute, Medecine Douce'+myDict["CATEGORY"]
        myDict["YEAR"] = '2022'
        myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//img[contains(@class, 'avatar--lg')]", 'src')), item["__img"])
        
        if check_exists_by_xpath(driverinstance, "//div[contains(@id, 'picture')]//div[contains(@class, 'img')][1]") == 0:
            myDict["IMAGE2"] = valueifnull(WebDriverWait(driverinstance, 20).until(EC.visibility_of_element_located((By.XPATH, "//div[contains(@id, 'picture')]//div[contains(@class, 'img')][1]"))).value_of_css_property("background-image"), item["__img"])
            myDict["IMAGE2"] = re.split('[()]',myDict["IMAGE2"])
            if len(myDict["IMAGE2"]) > 1:
                myDict["IMAGE2"] =  myDict["IMAGE2"][1]
        
        """if check_exists_by_xpath(driverinstance, "//div[contains(@id, 'picture')]//div[contains(@class, 'img')][2]") == 0:
            myDict["IMAGE3"] = valueifnull(WebDriverWait(driverinstance, 20).until(EC.visibility_of_element_located((By.XPATH, "//div[contains(@id, 'picture')]//div[contains(@class, 'img')][2]"))).value_of_css_property("background-image"), item["__img"])
            myDict["IMAGE3"] = (re.split('[()]',myDict["IMAGE3"])[1])"""
        ...
        myDict["EMAIL"] = ''
        if check_exists_by_xpath(driverinstance, '//a[contains(@title, "Appel")]') == 0:
            print(" APPEL " + str(count))
            button = driverinstance.find_element(By.XPATH, '//a[contains(@title, "Appel")]')
            driverinstance.execute_script("arguments[0].click();", button)
            myDict["PHONE"] = valueifnull(remove_tags(findATTR(driverinstance, '//a[contains(@title,"Appel")]', 'href')), " ")
            myDict["PHONE"] = myDict["PHONE"].replace('tel:','')
        myDict["FACEBOOK"] = ''
        myDict["TWITTER"] = ''
        myDict["INSTAGRAM"] = ''
        myDict["LINKEDIN"] = ''
        if check_exists_by_xpath(driverinstance, '//a[contains(@data-target, "session-infos")]') == 0:
            myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@data-target, "place-card__text")]'))
        
        myDict["CREATED_AT"] = ''
        myDict["UPDATED_AT"] = '2022'
        myDict["AVIS"] = random.randint(3, 5)
        myDict["AUTHOR"] = valueifnull(remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class,"review__header")]//span[1]')), ' ')
        myDict["AVIS_EMAIL"] =  myDict["AUTHOR"]+"gmail.com"
        myDict["COMMENT"] = valueifnull(remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class,"review__message")]')), ' ')
        
        append_new_line(r'result-medoucine.txt', str(myDict))
        final_result.append(myDict)


    with open("res-medoucine.json", "wb") as writeJSON:
