...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/choosemycompany.com-organic.Positions-fr-20230210-2023-02-12T02 58 05Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(15, 440):
        __url = str(df.at[i, 'URL'])
        if '/avis-certifies/' in __url and int(df.at[i, 'Keyword Difficulty']) < 46: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                
                __url = __url.replace("/salaries", "/infos" )
                __url = __url.replace("/stagiaires-alternants", "/infos" )
                __url = __url.replace("/offres-d-emploi", "/infos" )
                ...
                __title = __title.replace(" | choosemycompany", " " )
                __img = ""
                ...
                print(list_search)
                append_new_line(r'link-choosemycompany.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://choosemycompany.com/")
    time.sleep(8)
    recaptcha(driverinstance, '//button[@id="tarteaucitronPersonalize2"]')
    time.sleep(4)
    ...
    for item in list_indeed:
        ...
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "activities-content")][1]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = item["__description"]
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//span[contains(@class, "company-group")][1]'))
        if myDict["CATEGORY"] == ' ':
        ...
        myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//div[contains(@class, 'logo-and-participate-btn')]//img", 'src')), item["__img"])
        
        myDict["VIDEO"] = remove_tags(getinnertextXpath(driverinstance, '//iframe[contains(@src, "youtube")]'))
        
        myDict["WEBSITE"] = valueifnull(remove_tags(findATTR(driverinstance, "//a[contains(@class, 'company-website-link')]", 'href')), " ")
        myDict["EMAIL"] = ''
        myDict["PHONE"] = ''
        ...
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//address[contains(@class, "offers-company-adress")][1]//br'))
        ...
        append_new_line(r'result-choosemycompany.txt', str(myDict))
        final_result.append(myDict)


    with open("res-choosemycompany.json", "wb") as writeJSON:
