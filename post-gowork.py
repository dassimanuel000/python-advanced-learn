...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/gowork.fr-organic.Positions-fr-20230103-2023-01-04T11 21 47Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(10, 350):
        __url = str(df.at[i, 'URL'])
        if 'fr/' in __url and int(df.at[i, 'Traffic']) > 10:
            if '/blog/' in __url or '/avis/' in __url :
                pass
            else:
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("gowork", " " )
                __title = __title.replace(" | Reviews, Pricing, Features", " " )
                __title = __title.replace(" | gowork", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-gowork.txt', str(__url))
    
def popup(driver, xpath):
    delay = 3 
    try:
...
def browser():
    ...
    driver = webdriver.Chrome('/home/ds/env/selenium/2023/chromedriver_linux64/chromedriver',chrome_options=chrome_options)
    ...
    driver.get("https://www.gowork.com")
    recaptcha(driver, '//button[@class="button--medium button--green"]')
    time.sleep(4)
    input('/////////////////////////')
    for item in list_indeed:
        ...
        time.sleep(5)
        try:
            popup(driver, '//button[@class="button--medium button--green"]')
        except :
            pass
        myDict = {}
        myDict["TITRE"] = getinnertextXpath(driver, '//h2[contains(@class, "company-header__title")]')
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@id, "company-name-location")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "company-top-info")]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "company-tags")]//a[1]')) + remove_tags(getinnertextXpath(driver, '//div[contains(@class, "company-tags")]//a[2]')) 
        myDict["CATEGORY"] +=  myDict["CATEGORY"] +  'Compagnie,  Company, Entreprises'
        myDict["YEAR"] = '2022'
        try:
            img_src = ''
            wait = WebDriverWait(driver, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//div[contains(@class, "company-logo")]//div//img')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        myDict["WEBSITE"] = remove_tags(getinnertextXpath(driver, "//i[@class='icon-globe']/following-sibling::div"))
        myDict["EMAIL"] = ''
        myDict["PHONE"] = remove_tags(getinnertextXpath(driver, "//i[@class='icon-phone-call']/following-sibling::div"))
        myDict["FACEBOOK"] = ''
        myDict["TWITTER"] = ''
        myDict["INSTAGRAM"] = ''
        myDict["LINKEDIN"] = ''
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driver, "//i[@class='icon-home-new']/following-sibling::div"))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driver, "//i[@class='icon-calendar']/following-sibling::div"))
        
        append_new_line(r'result-gowork.txt', str(myDict))
        final_result.append(myDict)


    with open("res-gowork.json", "wb") as writeJSON:
