...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/influenster.com-backlinks_pages.xlsx')
    ...
    for i in range(10, 110):
        ...
        if '/reviews/' in __url:
            if 'edit-video' in __url:
                ...
                __title = __title.replace(" Reviews 2022", " " )
                __title = __title.replace("| Influenster", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-influenster.txt', str(__url))
...
def browser():
    ...
    driver.get("https://www.influenster.com/")
    ...
    for item in list_indeed:
        ...
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = remove_tags(getinnertextXpath(driver, '//h1[0]'))
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//p[@class="ProductDescription_product"][0]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//*[@id="__next"]/main/div[1]/nav/a[1]'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//*[@id="__next"]/main/div[1]/nav/a[2]'))
        myDict["YEAR"] = '2022'
        try:
            img_src = driver.find_element(By.XPATH, '(//div[@class="ProductImage_product-image__main-image__d__rb"]//div//span//img)')
            img_src = img_src.get_attribute('src')
        except NoSuchElementException:
        ...
        try:
            img_src = driver.find_element(By.XPATH, '(//div[@class="SecondaryProductImages_secondary-product-images__image-wrapper__ARg2m"]//div//span//img)')
            img_src = img_src.get_attribute('src')
        except NoSuchElementException:
            img_src = item["__title"]
            pass
        ...
        append_new_line(r'result-influenster.txt', str(myDict))
        final_result.append(myDict)


    with open("res-influenster.json", "wb") as writeJSON:
