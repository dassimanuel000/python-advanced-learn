...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/which.co.uk-backlinks_pages.xlsx')
    # Entrer les lignes à scrap
    for i in range(10, 100):
        __url = str(df.at[i, 'Source url'])
        if '/reviews' in __url or '/news' in __url:
            __title = str(df.at[i, 'Source title'])
            __title = __title.replace(" – Which? News", "" )
            __title = __title.replace(" - which.", " " )
            list_search.append({'__url': __url, '__title': __title})
            print(list_search)
            append_new_line(r'link-which.txt', str(__url))
...
def browser():
    ...
    driver.get("https://www.which.com/")
    recaptcha(driver, '//button[@id="onetrust-accept-btn-handler"]')
    for item in list_indeed:
        driver.get(item["__url"])
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = Translator().translate(text=getinnertextXpath(driver, '//h1'), dest='fr').text
        print(myDict["TITRE"])
        myDict["url"] = item["__url"]
        DESCRIPTION1 = ''
        for b in range(1, 5):
            DESCRIPTION1 +=  remove_tags(getinnertextXpath(driver, "//h3[@data-testid='typography-title-400']["+str(b)+"]"))
        for b in  range(1, 5):
            DESCRIPTION1 +=  remove_tags(getinnertextXpath(driver, "//h3[@data-testid='typography-title-600']["+str(b)+"]"))
        myDict["DESCRIPTION"]= str(DESCRIPTION1)
        myDict["CATEGORY"] =  getinnertextXpath(driver, '//a[@role="navigation"]//a//span//span')
        myDict["YEAR"] = '2022'
        myDict["VIDEO"] = ''
        
        myDict["LOCATION"] =  'Online'
        myDict["CREATED_AT"] =  getinnertextXpath(driver, '//time[@data-testid="typography-very-small-print"]')

        try:
            img_src = driver.find_element(By.XPATH, '(//img[@data-testid="picture-container-image"][1])')
            img_src = img_src.get_attribute('src')
        except NoSuchElementException:
        ...
        try:
            img_src = driver.find_element(By.XPATH, '(//img[@data-testid="picture-container-image"][2])')
            img_src = img_src.get_attribute('src')
        except NoSuchElementException:
            img_src = item["__title"]
            pass
        ...
        #print(myDict)
        append_new_line(r'result-which.txt', str(myDict))
        final_result.append(myDict)


    with open("res-which.json", "wb") as writeJSON:
