...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/glassdoor.com-backlinks_pages.xlsx')
    # Entrer les lignes à scrap
    for i in range(100, 250):
        __url = str(df.at[i, 'Source url'])
        if __url == 'http://www.glassdoor.com/Overview/index.htm':
            continue  
        """if '/Reviews' in __url or '/Salary' in __url or '/Jobs' in __url or '/Interview' in __url or '/Benefits' in __url:
            __url = __url.replace("/Reviews", "/Overview" )
            __url = __url.replace("/Salary", "/Overview" )
            __url = __url.replace("/Jobs", "/Overview" )
            __url = __url.replace("/Interview", "/Overview" )
            __url = __url.replace("/Benefits", "/Overview" )"""
        if '/Overview' in __url or '/Benefits' in __url:
            __title = str(df.at[i, 'Source title'])
            __title = __title.replace("Working at ", "" )
            __title = __title.replace(" | Glassdoor", " " )
            list_search.append({'__url': __url, '__title': __title})
            print(list_search)
            append_new_line(r'link-glassdoor.txt', str(__url))
...
def browser():
    ...
    for item in list_indeed:
        #urlParent = 'https://fr.indeed.com/jobs?q='++'&l='+ item["__title"]
        driver.get(item["__url"])
        tryAndRetryClickXpath(driver, '//a[@class="sqLogoLink"]')
        time.sleep(4)
        
        try:
            img_src = driver.find_element(By.XPATH, '(//span[@class="sqLogo tighten lgSqLogo logoOverlay"]//img)')
        ...
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//span[@data-test="employerDescription"]'))
        myDict["CATEGORY"] =  getinnertextXpath(driver, '//a[@data-test="employer-industry"]')
        try:
            myDict["YEAR"] = (getinnertextXpath(driver, '//div[@data-test="employer-founded"]'))
        except NoSuchElementException:
            pass
        myDict["IMAGE1"] = str(img_src)
        try:
            myDict["VIDEO"] =  getinnertextXpath(driver, '//iframe[@data-test="videoFrame"]')
        except NoSuchElementException:
            myDict["VIDEO"] =  item["__title"]
            pass
        
        myDict["LOCATION"] =  getinnertextXpath(driver, '//div[@data-test="employer-headquarters"]')
        myDict["CREATED_AT"] =  getinnertextXpath(driver, '//div[@data-test="employer-founded"]')

        #print(myDict)
        append_new_line(r'result-glassdoor.txt', str(myDict))
        final_result.append(myDict)


    with open("res-glassdor.json", "wb") as writeJSON:
