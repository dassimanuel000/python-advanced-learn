...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/signal-arnaques.com-organic.Positions-fr-20230103-2023-01-04T11 23 43Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(10, 150):
        __url = str(df.at[i, 'URL'])
        if '/scam/view/' in __url and int(df.at[i, 'Traffic']) > 9:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("signal-arnaques", " " )
                __title = __title.replace(" | Reviews, Pricing, Features", " " )
                __title = __title.replace(" | signal-arnaques", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-signal-arnaques.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    driverinstance = webdriver.Chrome('/home/ds/env/selenium/2023/chromedriver_linux64/chromedriver',chrome_options=chrome_options)
    ...
    driverinstance.get("https://www.signal-arnaques.com")
    ...
    for item in list_indeed:
        ...
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = remove_tags(getinnertextXpath(driverinstance, '//title')).split('|')[0]
        if myDict["TITRE"] == "www.signal-arnaques.fr":
            myDict["TITRE"] = item["__title"].split(',')[0]
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//*[@id="w0"]/tbody/tr[4]/td'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//h1'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//ul[contains(@class, "breadcrumb")]//li[2]//a')) + ','
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//a[contains(@href, "/cat/")][1]'))
        myDict["YEAR"] = remove_tags(getinnertextXpath(driverinstance, '//*[@id="w0"]/tbody/tr[1]/td'))
        
        myDict["IMAGE1"] = str('https://i.ibb.co/wspgZ6B/image-blank.png')
        ...
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driverinstance, '//*[@id="w0"]/tbody/tr[1]/td'))
        
        append_new_line(r'result-signal-arnaques.txt', str(myDict))
        final_result.append(myDict)


    with open("res-signal-arnaques.json", "wb") as writeJSON:
