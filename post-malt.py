...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/malt.fr-organic.Positions-fr-20230103-2023-01-04T12 28 00Z.xlsx')
...
def browser():
    ...
    driverinstance.get("https://www.malt.com")
    ...
    for item in list_indeed:
        ...
        if myDict["TITRE"] == "www.malt.fr":
            myDict["TITRE"] = item["__title"].split(',')[0]
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//section[contains(@data-alert-section, "DESCRIPTION")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//section[contains(@id, "profileSkillsSection")]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//ul[contains(@class, "profile-page-categories__list")]//li[1]//a'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = substring_after(item["__title"], ',')
        myDict["CATEGORY"] +=  myDict["CATEGORY"] + ", Influenceur, Influencer, Freelance"
        myDict["YEAR"] = '2022'
        try:
            wait = WebDriverWait(driverinstance, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//img[contains(@class, "profile-picture-image")]')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        myDict["VIDEO"] = ''
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//dd[contains(@data-profile-location-preference-address, "")]'))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driverinstance, '//span[contains(@data-experience-date, "")]'))
        
        append_new_line(r'result-malt.txt', str(myDict))
        final_result.append(myDict)


    with open("res-malt.json", "wb") as writeJSON:
