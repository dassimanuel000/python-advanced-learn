...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/maformation.fr-organic.Positions-fr-20230106-2023-01-07T03 31 41Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(1, 3505):
        __url = str(df.at[i, 'URL'])
        if '/centres/' in __url and int(df.at[i, 'Traffic']) > 4:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("maformation", " " )
                __title = __title.replace(" | Reviews, Pricing, Features", " " )
                __title = __title.replace(" | maformation", " " )
                __img = "https://www.capemploi-69.com/assets/cache/images-cache/images/cheops/commun/bac-a-sable/vignette05.ab5f1aa5.jpg"
                __description = __title + "qui fournit les cours particuliers, le E-learning et les examens certifiantes. Nous proposons une gamme de programmes et nos propres méthodes éprouvées pour vous aider dans votre développement."
                list_search.append({'__url': __url, '__title': __title, '__img': __img, '__description': __description})
                print(list_search)
                append_new_line(r'link-maformation.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://www.maformation.fr/centres/annuaire")
    time.sleep(4)
    recaptcha(driverinstance, '//button[@id="hw-cc-notice-accept-btn"]')
    time.sleep(4)
    """for i in range(1, 5):
        driverinstance.get("https://www.maformation.fr/centres/annuaire?PageNumber="+ str(i) +"")
        time.sleep(5)
        wait = WebDriverWait(driverinstance, 10)
        elems = driverinstance.find_elements_by_xpath('//a[contains(@class, "card-brand__inner")]')
        #elems =  driverinstance.find_elements_by_xpath('//a[contains(@class, "card-brand__inner")]')
        for elem in elems:
            __url = (elem.get_attribute("href"))
            __title = remove_tags(getinnertextXpath(elem, '//h3[contains(@class, "card-brand__title")]'))
            __img = findATTR(elem, '//div//img', 'src')
            __description = remove_tags(getinnertextXpath(elem, '//div[contains(@class, "card-brand__description")]'))
            list_search.append({'__url': __url, '__title': __title, '__img': __img, '__description': __description})
        print(list_search)"""
    ...
    for item in list_indeed:
        ...
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = remove_tags(getinnertextXpath(driverinstance, '//h1[contains(@class, "header-card__title")]'))
        if myDict["TITRE"] == ' ':
            myDict["TITRE"] = item["__title"]
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '/html/body/main/article/div/div/section[1]/div/div/div/p'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = item["__description"]
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@data-cy, "brand-detail-tag")]'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//h3[contains(@data-cy, "card-training-title")][1]'))
        myDict["CATEGORY"] += ', CPF, Centre de formation, FORMATION, Compagnie,  Company, Entreprises'
        myDict["YEAR"] = '2022'
        myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//div[contains(@class, 'header-card__logo')]//img", 'src')), item["__img"])
        myDict["IMAGE2"] = remove_tags(findATTR(driverinstance, "//div[contains(@class, 'list-certification__inner')]//figure//img", 'src'))
        
        myDict["VIDEO"] = remove_tags(findATTR(driverinstance, "//div[contains(@data-video-card-video-provider, 'youtube')]", 'data-video-card-video-id'))
        myDict["VIDEO"] = 'https://www.youtube.com/watch?v='+ myDict["VIDEO"]
        myDict["WEBSITE"] = remove_tags(findATTR(driverinstance, "//a[contains(@rel, 'noopener')]", 'href'))
        myDict["EMAIL"] = ''
        myDict["PHONE"] = ''
        myDict["FACEBOOK"] = ''
        myDict["TWITTER"] = ''
        myDict["INSTAGRAM"] = ''
        myDict["LINKEDIN"] = ''
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@data-cy, "brand-detail-location")]'))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driverinstance, '//li[contains(@href, "creationDate")]//span[2]'))
        myDict["UPDATED_AT"] = '2022'
        
        append_new_line(r'result-maformation.txt', str(myDict))
        final_result.append(myDict)


    with open("res-maformation.json", "wb") as writeJSON:
