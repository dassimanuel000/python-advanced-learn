...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/amazon.fr-backlinks_pages.xlsx')
    ...
    for i in range(60, 300):
        ...
        if '/dp/' in __url:
            if 'edit-video' in __url:
                ...
                __title = __title.replace("Amazon.fr", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-amazon_fr.txt', str(__url))
    ...
    for item in list_indeed:
        ...
        if 'Appstore for Android' in getinnertextXpath(driver, '//title'):
            ...
            if '" "' in myDict["IMAGE1"]:
                ...
                append_new_line(r'result-amazon_fr.txt', str(myDict))
                final_result.append(myDict)


    with open("res-amazon_fr.json", "wb") as writeJSON:
