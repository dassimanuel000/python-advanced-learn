...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/trustradius.com-backlinks_pages.xlsx')
    ...
    for i in range(10, 210):
        ...
        if '/reviews' in __url:
            if 'edit-video' in __url:
                ...
                __title = __title.replace("Reviews & Ratings 2022", " " )
                __title = __title.replace(" | Reviews, Pricing, Features", " " )
                __title = __title.replace(" | TrustRadius", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-trustradius.txt', str(__url))
...
def browser():
    ...
    driver.get("https://www.trustradius.com")
    ...
    for item in list_indeed:
        ...
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "ProductDetailsSection_body")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "ProductDescription_description")]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//ul[contains(@class, "ProductPageHeader_info")]//li[2]//a'))
        myDict["YEAR"] = '2022'
        try:
            wait = WebDriverWait(driver, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//div[contains(@class, "ProductPageHeader_logo")]//img')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        try:
            wait = WebDriverWait(driver, 10)
            try:
                video_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//iframe')))
        ...
        append_new_line(r'result-trustradius.txt', str(myDict))
        final_result.append(myDict)


    with open("res-trustradius.json", "wb") as writeJSON:
