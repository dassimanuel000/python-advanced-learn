...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/welcometothejungle.com-organic.Positions-fr-20230210-2023-02-12T02 04 35Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(200, 2600):
        __url = str(df.at[i, 'URL'])
        if '/companies/' in __url and int(df.at[i, 'Keyword Difficulty']) < 34: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                
                __title = str(df.at[i, 'Keyword'])
                __title = __title.replace("welcometothejungle", " " )
                ...
                print(list_search)
                append_new_line(r'link-welcometothejungle.txt', str(__url))
            else:
                pass
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://welcometothejungle.com/")
    time.sleep(8)
    recaptcha(driverinstance, '//button[@id="axeptio_btn_acceptAll"]')
    time.sleep(4)
    ...
    for item in list_indeed:
        ...
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@data-testid, "organization-content-block-text")][1]/article//p[1]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = item["__description"]
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//ul[contains(@data-testid, "organization-cover-metas")]//li[1]'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = 'StartUp'
        myDict["CATEGORY"] += ' , Compagnie,  Company, Entreprises'
        myDict["YEAR"] = '2022'
        tempp= item["__url"] + '/out/company-logo'
        #myDict["IMAGE1"] = WebDriverWait(driverinstance, 20).until(EC.presence_of_element_located((By.XPATH, "//div[contains(@data-testid, 'organization-header-logo-main')]"))).value_of_css_property("background-image")
        
        try:
            myDict["IMAGE1"] = driverinstance.find_element(By.XPATH, "//div[contains(@data-testid, 'organization-header-logo-main')]")
            myDict["IMAGE1"] = myDict["IMAGE1"].value_of_css_property('background-image')
            myDict["IMAGE1"] =  str(myDict["IMAGE1"].lstrip('url("').rstrip('")'))
        except NoSuchElementException:
            myDict["IMAGE1"] = ''
            pass
        ...
        myDict["WEBSITE"] = valueifnull(remove_tags(findATTR(driverinstance, "//ul[contains(@data-testid, 'organization-cover-metas')]//li[3]//a[contains(@target, '_blank')]", 'href')), " ")
        myDict["EMAIL"] = ''
        myDict["PHONE"] = ''
        ...
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//ul[contains(@data-testid, "organization-cover-metas")]//li[2]'))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "company-stats")]//article//ul//li[1]//span'))
        myDict["UPDATED_AT"] = '2022'
        
        append_new_line(r'result-welcometothejungle.txt', str(myDict))
        final_result.append(myDict)


    with open("res-welcometothejungle.json", "wb") as writeJSON:
