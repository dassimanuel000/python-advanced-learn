...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/franceverif_fr_organic_Positions_fr_20230227_2023_02_28T08_33_20Z.xlsx')
    # Entrer les lignes à scrap
    for i in range(6000, 9999):
        __url = str(df.at[i, 'URL'])
        if '/site/' in __url and int(df.at[i, 'Keyword Difficulty']) < 36: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                ...
                append_new_line(r'all-franceverif.txt', str(myDict))
            else:
                pass
