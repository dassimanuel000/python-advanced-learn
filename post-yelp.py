...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/yelp.com-backlinks_pages.xlsx')
    # Entrer les lignes à scrap
    for i in range(5, 80):
        __url = str(df.at[i, 'Source url'])
        if '/biz' in __url:
            __title = str(df.at[i, 'Source title'])
            __title = __title.replace(" - Phone Number - Yelp", "" )
            __title = __title.replace(" - Yelp", " " )
            list_search.append({'__url': __url, '__title': __title})
            print(list_search)
            append_new_line(r'link-yelp.txt', str(__url))
...
def browser():
    ...
    for item in list_indeed:
        ...
        myDict["DESCRIPTION"] = driver.find_element(By.XPATH, '//meta[@name="description"]')
        myDict["DESCRIPTION"] = myDict["DESCRIPTION"].get_attribute('content')
        try:
            myDict["CATEGORY"] =  getinnertextXpath(driver, '/html/body/yelp-react-root/div[1]/div[3]/div[1]/div[1]/div/div/span[3]/span[1]/a')
        except NoSuchElementException:
            myDict["CATEGORY"] =  getinnertextXpath(driver, '/html/body/yelp-react-root/div[1]/div[3]/div[1]/div[1]/div/div/span[2]/span[1]/a')
            pass
        myDict["YEAR"] = '2022'
        myDict["VIDEO"] = ''
        
        myDict["LOCATION"] =  getinnertextXpath(driver, '/html/body/yelp-react-root/div[1]/div[4]/div/div/div[2]/div/div[2]/div/aside/section[1]/div/div[3]/div/div[1]/p[2]')
        myDict["CREATED_AT"] =  '2022'
        try:
            tryAndRetryClickXpath(driver, '//a[@href="biz_redir"][2]')
            time.sleep(5)
            img_src = findlogo(driver)
            input(img_src)
        except NoSuchElementException:
            img_src = getinnertextXpath(driver, '//a[@href="biz_redir"][2]')
            pass
        myDict["IMAGE1"] = str(img_src)
        #print(myDict)
        append_new_line(r'result-yelp.txt', str(myDict))
        final_result.append(myDict)


    with open("res-yelp.json", "wb") as writeJSON:
