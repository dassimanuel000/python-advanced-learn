...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/fiverr.com-backlinks_pages.xlsx')
    # Entrer les lignes à scrap
    for i in range(10, 210):
        __url = str(df.at[i, 'Source url'])
        __title = str(df.at[i, 'Source title'])
        if '| Profile | ' in __title:
            if 'categories' in __url:
                pass
            else:    
                __title = __title.replace(" | Profile | Fiverr", " " )
                __title = __title.replace(" | Fiverr", " " )
                __title = __title.replace("fiverr", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-fiverr.txt', str(__url))
...
def browser():
    ...
    for item in list_indeed:
        ...
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = getinnertextXpath(driver, '//div[contains(@class, "username-line")]')
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "description")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driver, '//div[contains(@class, "seller-profile")]'))
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driver, '//a[contains(@href, "hire")]'))
        myDict["CATEGORY"] +=  myDict["CATEGORY"] + ", Influenceur, Influencer, Freelance"
        myDict["YEAR"] = '2022'
        try:
            wait = WebDriverWait(driver, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//label[contains(@class, "profile-pict")]//img')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        try:
            wait = WebDriverWait(driver, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//div[contains(@class, "slide-item active")]//img')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        video_src = item["__title"]
        myDict["VIDEO"] = video_src
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driver, '//li[contains(@class, "location")]//b'))
        
        myDict["CREATED_AT"] = remove_tags(getinnertextXpath(driver, '//li[contains(@class, "member-since")]//b'))
        
        append_new_line(r'result-fiverr.txt', str(myDict))
        final_result.append(myDict)


    with open("res-fiverr.json", "wb") as writeJSON:
