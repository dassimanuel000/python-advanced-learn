...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/freelancer.com-backlinks_pages.xlsx')
    ...
    for i in range(1, 510):
        ...
        if '/u/' in __url:
            if __url in 'merdee' :
                ...
                __title = __title.replace("freelancer", " " )
                __title = __title.replace(" - freelancer Freelancer ", " " )
                __title = __title.replace(" | freelancer", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-freelancer.txt', str(__url))
...
def browser():
    ...
    driverinstance.get("https://www.freelancer.com")
    ...
    for item in list_indeed:
        ...
        time.sleep(5)
        myDict = {}
        myDict["TITRE"] = remove_tags(getinnertextXpath(driverinstance, '//fl-heading[contains(@class, "Username-displayName")]')) + ' ' + remove_tags(getinnertextXpath(driverinstance, '//fl-heading[contains(@class, "Username-userId")]'))
        if myDict["TITRE"] == "www.freelancer.fr" or myDict["TITRE"] == " ":
            myDict["TITRE"] = item["__title"].replace('https://www.freelancer.com/freelancers/', '').replace('/', '')
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//fl-text[contains(@class, "Content")]'))
        myDict["CATEGORY"] = ", Influenceur, Influencer, Freelance"
        myDict["YEAR"] = '2022'
        try:
            wait = WebDriverWait(driverinstance, 10)
            try:
                img_src = wait.until(EC.visibility_of_element_located((By.XPATH, '//img[contains(@alt, "User Avatar")]')))
                img_src = img_src.get_attribute('src')
            except:
                pass
        except:
        ...
        myDict["VIDEO"] = ''
        myDict["LOCATION"] = remove_tags(getinnertextXpath(driverinstance, '//fl-col[contains(@class, "SupplementaryInfo")][1]'))
        
        myDict["CREATED_AT"] = "2023"
        
        append_new_line(r'result-freelancer.txt', str(myDict))
        final_result.append(myDict)


    with open("res-freelancer.json", "wb") as writeJSON:
