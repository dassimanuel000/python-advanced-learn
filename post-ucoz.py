...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/s/ucoz.fr.xlsx')
    # Entrer les lignes à scrap
    for i in range(1, 500):
        __url = str(df.at[i, 'Pages les plus populaires'])
        if '/avis/' in __url and int(df.at[i, 'Impressions']) > 92: # and int(df.at[i, 'Keyword Difficulty']) < 35 and int(df.at[i, 'Search Volume']) > 7000:
            if True:
                
                __title = str(df.at[i, 'Pages les plus populaires'])
                __title = __title.replace("https://ucoz.fr/avis/", " " )
                __title = __title.replace("https://ucoz.fr/places/", " " )
                __title = __title.replace("-", " " )
                __img = ""
                __description = __title + "  <br> Évaluation de fiabilité du site "+ __title +" : définition    <br>  Les caractéristiques du site "+ __title +"   <br>  Le business plan de "+ __title +"     <br> Le financement de "+ __title +"    <br>  La forme juridique de "+ __title +""
                ...
                myDict["DESCRIPTION"] = myDict["TITRE"] +"  <br> " + __description
                
                myDict["TITRE"] = (__url).replace("https://ucoz.fr/avis/", "").replace("https://ucoz.fr/place/", "").replace(".", " ").replace("-", " ")
                myDict["TITRE"] = valueifnull(myDict["TITRE"].title(), __title)
                myDict["url"] = __url
                ...
                myDict["WEBSITE"] = (__url).replace("https://ucoz.fr/avis/", "").replace("https://ucoz.fr/places/", " " )
                
                
                myDict["EMAIL"] = "contact@"+(__url).replace("https://ucoz.fr/avis/", "").replace("https://ucoz.fr/places/", " " )
                ...
                append_new_line(r'all-ucoz.txt', str(myDict))
            else:
                pass
    

    with open("res-ucoz.json", "wb") as writeJSON:
...
def browser():
    ...
    time.sleep(2)
    driverinstance.get("https://ucoz.fr/")
    time.sleep(8)
    ...
    for item in list_indeed:
        ...
        myDict["TITRE"] = myDict["TITRE"].title()
        myDict["TITRE"] = myDict["TITRE"].replace(" avis ", " " ).replace(" avis", " " ).replace(" Avis ", " " ).replace(" Avis", " " )
        myDict["url"] = item["__url"]
        myDict["DESCRIPTION"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "theiaStickySidebar")]'))
        if myDict["DESCRIPTION"] == ' ':
            myDict["DESCRIPTION"] = item["__description"]
        
        myDict["CATEGORY"] = remove_tags(getinnertextXpath(driverinstance, '//div[contains(@class, "fl-places-average-cat")]'))
        if myDict["CATEGORY"] == ' ':
            myDict["CATEGORY"] = ' Company'
        myDict["YEAR"] = '2022'
        
        myDict["IMAGE1"] = valueifnull(remove_tags(findATTR(driverinstance, "//div[contains(@class, 'fl-places-logo-contain')]//img", 'src')), item["__img"])
            
        myDict["VIDEO"] = ' '
        
        myDict["WEBSITE"] = valueifnull(remove_tags(getinnertextXpath(driverinstance, '//span[contains(@class, "fl-places-meta-website")]//a')), " ")
        myDict["EMAIL"] = remove_tags(getinnertextXpath(driverinstance, '//span[@class="fl-places-meta-email"]'))
        myDict["PHONE"] =  remove_tags(getinnertextXpath(driverinstance, '//span[@class="fl-places-meta-phone"]'))
        myDict["FACEBOOK"] = " "
        myDict["TWITTER"] = " "
        myDict["INSTAGRAM"] = " "
        myDict["LINKEDIN"] =  " "
        ...
        append_new_line(r'result-ucoz.txt', str(myDict))
        final_result.append(myDict)
        print(myDict)
