...
def RefreshExcel():
    ...
    df = pd.read_excel(r'/home/ds/Documents/LIFE/data/Life-cm/g2.com-backlinks_pages.xlsx')
    # Entrer les lignes à scrap
    for i in range(10, 110):
        __url = str(df.at[i, 'Source url'])
        if '/products/' in __url:
            if 'edit-video' in __url:
                pass
            else:    
                __title = str(df.at[i, 'Source title'])
                __title = __title.replace("Reviews 2022: Details, Pricing, & Features | G2", " " )
                __title = __title.replace(" | G2", " " )
                list_search.append({'__url': __url, '__title': __title})
                print(list_search)
                append_new_line(r'link-g2.txt', str(__url))
...
def browser():
    ...
    for item in list_indeed:
        ...
        append_new_line(r'result-g2.txt', str(myDict))
        final_result.append(myDict)


    with open("res-g2.json", "wb") as writeJSON:
