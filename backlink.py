...
def post_commet(driverinstance, link, title, url):
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//textarea[contains(@id, "comment")]') == 0:
        try:
            ...
            tryAndRetryFillByXpath(driverinstance, '//form[contains(@id, "comment")]//textarea[contains(@id, "comment")]', str('Je tenais à prendre un moment pour vous féliciter chaleureusement pour votre article inspirant et captivant !'))
        except NoSuchElementException:
            pass
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@id, "email")]') == 0:
        try:
            tryAndRetryFillByXpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@id, "email")]', str('contact@therapeute.net'))
        except NoSuchElementException:
            pass
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@id, "author")]') == 0:
        try:
            tryAndRetryFillByXpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@id, "author")]', str('Therapeute'))
        except NoSuchElementException:
            pass
    if check_exists_by_xpath(driverinstance, '//form[contains(@id, "comment")]//input[contains(@id, "url")]') == 0:
        try:
            driverinstance.find_element(By.XPATH, '//form[contains(@id, "comment")]//input[contains(@id, "url")]').send_keys(url)
        except NoSuchElementException:
            pass
...
def publish():
    ...
    word_file = r"D:/Documents/Website/therapeute.net/POSTEUR/words_alpha.txt"
    ...
    for item in range(last_number + 1, 8400):
        my_list = list()
        driverinstance.get("https://google.com")
        ...
        if "https://therapeute.net/" in title:
            title = title
        else:
            title = WORDS
        if "/wp-content/"  in title  or "listivotheme"  in title or "data:image"  in title or "/listivo_template/"  in title or "/wp-admin/"  in title:
            title = WORDS            
        title = title.replace('https://therapeute.net/avis/', '').replace('https://therapeute.net/review/', '').replace('https://therapeute.net/category/', '').replace('https://therapeute.net/tag/', '').replace('https://', '').replace('/', '').replace('.fr', '')
                    ...
                    if get_website_status(hostname+"wp-admin/") == "200":
                        ...
                        if "wp-login" in driverinstance.page_source or check_exists_by_xpath(driverinstance, '//*[contains(@id, "login")]') == 0:
                            ...
                            if check_exists_by_xpath(driverinstance, '//input[contains(@id, "author")]') == 0:
                                post_commet(driverinstance, hostname, title, str(linecache.getline("D:/Documents/Website/therapeute.net/POSTEUR/backlink.txt", item)).replace("\n", ""))
                            else:
        ...
        append_new_line(r'all_link_googlebot_done.txt', str(post_link))
...
#get_website_status("")import random
